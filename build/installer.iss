...
; Statistics database initialization script
Source: "{#SourcePath}\..\build\init-stats-db.sql"; DestDir: "{app}"; Flags: ignoreversion
...
begin
  ...
  begin
    // Initialize usage statistics database
    StatsDbPath := ExpandConstant('{app}\plexprerollmanager.db');
    if not FileExists(StatsDbPath) then
    begin
      // Create empty database file
      SaveStringToFile(StatsDbPath, '', False);

      // Initialize database tables using SQLite command line
      Exec(ExpandConstant('{sys}\cmd.exe'), '/c echo CREATE TABLE IF NOT EXISTS PrerollUsage (Id INTEGER PRIMARY KEY AUTOINCREMENT, PrerollId TEXT NOT NULL, CategoryName TEXT NOT NULL, PlayStartTime DATETIME NOT NULL, PlayDuration INTEGER, ClientIp TEXT, UserAgent TEXT, PlexClientId TEXT, CreatedDate DATETIME DEFAULT CURRENT_TIMESTAMP); | sqlite3.exe "' + StatsDbPath + '"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Exec(ExpandConstant('{sys}\cmd.exe'), '/c echo CREATE TABLE IF NOT EXISTS DailyStats (Id INTEGER PRIMARY KEY AUTOINCREMENT, Date DATE NOT NULL, CategoryName TEXT NOT NULL, TotalPlays INTEGER DEFAULT 0, TotalWatchTime INTEGER DEFAULT 0, UniquePrerolls INTEGER DEFAULT 0, CreatedDate DATETIME DEFAULT CURRENT_TIMESTAMP, UpdatedDate DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(Date, CategoryName)); | sqlite3.exe "' + StatsDbPath + '"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Exec(ExpandConstant('{sys}\cmd.exe'), '/c echo CREATE INDEX IF NOT EXISTS idx_preroll_usage_preroll_id ON PrerollUsage(PrerollId); | sqlite3.exe "' + StatsDbPath + '"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Exec(ExpandConstant('{sys}\cmd.exe'), '/c echo CREATE INDEX IF NOT EXISTS idx_preroll_usage_category ON PrerollUsage(CategoryName); | sqlite3.exe "' + StatsDbPath + '"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Exec(ExpandConstant('{sys}\cmd.exe'), '/c echo CREATE INDEX IF NOT EXISTS idx_preroll_usage_start_time ON PrerollUsage(PlayStartTime); | sqlite3.exe "' + StatsDbPath + '"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Exec(ExpandConstant('{sys}\cmd.exe'), '/c echo CREATE INDEX IF NOT EXISTS idx_preroll_usage_date ON PrerollUsage(date(PlayStartTime)); | sqlite3.exe "' + StatsDbPath + '"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
    end;
    ...
    begin
      ...
      MsgBox('PlexPrerollManager v{#MyAppVersion} has been' + UpgradeMessage + ' as a Windows service and started automatically.' + #13#10 +
             'You can access it at: http://localhost:8089' + #13#10 +
             'New in v2.2.0: Usage statistics and analytics dashboard!' + ConfigPreserved, mbInformation, MB_OK);
    end else begin
      MsgBox('PlexPrerollManager v{#MyAppVersion} has been' + UpgradeMessage + '.' + #13#10 +
             'You can start it manually by running the executable or access it at: http://localhost:8089' + #13#10 +
             'New in v2.2.0: Usage statistics and analytics dashboard!' + ConfigPreserved, mbInformation, MB_OK);
    end;
  end;
end;
